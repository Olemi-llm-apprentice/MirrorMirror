    ...
    selectGenre: async ({ genre_id }: { genre_id: string }): Promise<string> => {
      ...
      try {
        ...
        if (coordinateList.length > 0) {
          ...
          const total = (firstCoord.items || []).reduce((sum: number, item: Item) => sum + item.price, 0);
          return JSON.stringify({
            success: true,
            coordinate_name: firstCoord.name,
            items_count: firstCoord.items?.length || 0,
            total_price: total,
          });
        }
